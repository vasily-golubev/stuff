...
\aqns[fldi]{
  ...
  \fld^s_\alpha\prn{\vr',t'} = \int_V\int_{-\infty}^{+\infty} \brc{\Delta\lamop\prn{\vr}\brk{\fld^i\prn{\vr,t}+\fld^s\prn{\vr,t}}}\cdot\\\cdot\gr_\alpha dtdV . 
  }
...
\aqns{
  \prn{\vd_1\cdot\vd_2} = \int_{S}\int_T \vd_1\prn{\vr',t}\cdot\vd_2\prn{\vr',t} dtdS
  }
...
\eqns[migr]{
  \Delta c_{\alpha,migr}^2\prn{\vr} = \int_{S}\int_T\int_{-\infty}^{+\infty} \brc{\Lapl \fld^i_\alpha\prn{\vr,t}}\cdot\gr_{}\cdot \\ \cdot\vd\prn{\vr',t'}dtdt'dV' .
  }
...
которое для \emph{хороших} импульсов упрощается до \(f\prn{t'}\), а для постоянного вектора $\vec{f}$ и произвольной функции координат \(g\prn{\vr}\) справедливо
\aqns{
...
\aqns[forwfin]{
  ...
  \int_V \Delta c_\beta^2\prn{\vr} \times \\ \times \frac{
      f\prn{t'-s_{\beta,b}\brs{\vr_0-\vr}-s_{\alpha,b}\brs{\vr'-\vr}}
    }{
    16\pi^2 c_{\beta,b}^2\brs{\vr_0-\vr}\brs{\vr'-\vr}
    }\vec{f} dV ,
...
\eqns[migrfin]{
  \Delta c_{\beta,migr}^2\prn{\vr} = \sum_\alpha \int_{V'}\int_T \inv{\rho\prn{\vr_0}} \vec{d}\prn{\vr',t'}\cdot \\ \cdot\Dop_\alpha'\Dop_\beta^0
    \frac{
      f\prn{t'-s_{\beta,b}\brs{\vr_0-\vr}-s_{\alpha,b}\brs{\vr'-\vr}}
    }{
    16\pi^2 c_{\beta,b}^2\brs{\vr_0-\vr}\brs{\vr'-\vr}
...
\aqns[forwfinh]{
  ...
  \Dop_\alpha'\int_V \Delta c_\beta^2\prn{\vr}\sum_{\vr_1 \in \{\vr,\,\mirr{\vr}\}} k_{\vr_1} \times \\ \times
    (
      \Dop^0_\beta\frac{
...
\aqns[migrfinh]{
  \Delta c_{\beta,migr}^2\prn{\vr} = \sum_\alpha \int_{V'}\int_T \inv{\rho\prn{\vr_0}} \vec{d}\prn{\vr',t'}\cdot\Dop_\alpha'\sum_{\vr_1 \in \{\vr,\,\mirr{\vr}\}} k_{\vr_1} \times \\ \times
    (
      \Dop^0_\beta\frac{
        f\prn{t'-s_{\beta,b}\brs{\vr_0-\vr}-s_{\alpha,b}\brs{\vr'-\vr_1}}
      }{
...
\begin {enumerate}
\item{Напишите программу, реализующую численное решение прямой сейсмической задачи для случая полупространства.}
\item{Напишите программу для построения миграционного изображения для случая полупространства.}
\end {enumerate}
...
К сожалению, аналитически либо полуаналитически она может быть найдена лишь для простейших случаев, таких как однородная среда, слоистая среда.
...
\item{Протестируйте написанную программу на аналитических решениях и на численных решениях, полученных в приближении Борна.}
\end {enumerate}
\clearpage
